...
public:
    ...
    static void translateLineToPoint(vec3& linePoint1, vec3& linePoint2, vec3 point) {
        ...
        // Egyenes egyik v�gpontj�nak �s az adott pontnak a k�l�nbs�ge
        vec3 oldMidPoint = (linePoint1 + linePoint2) / 2.0f; // Az eredeti egyenes k�z�ppontja
        vec3 translation = point - oldMidPoint; // Az eltol�si vektor

        linePoint1 =linePoint1 + translation; // Az egyenes els� v�gpontj�nak eltol�sa
    ...
    static vec3 intersection(vec3 p1, vec3 p2, vec3 p3, vec3 p4){
        ...
        if (det == 0) {
            return vec3(0, 0, 0); // Egyenesek p�rhuzamosak, nincs metsz�spont
        }
        ...
        if (t >= 0 && t <= 1 && s >= 0 && s <= 1) {
            ...
            return vec3(0, 0, 0); // A k�t szakasz nem metszi egym�st
        }
    }
...
public:
    ...
    int near_line(vec3 p)
            {
                ...
                int nearestIndex = -1; // Az index, ahol a legk�zelebbi egyenes tal�lhat�

                for (int i = 0; i < lines.Vtx().size(); i += 2) { // Minden m�sodik pont egy egyenes kezd�pontja
                    vec3 linePoint1 = lines.Vtx()[i];
                    vec3 linePoint2 = lines.Vtx()[i + 1];

                    // Az egyenesre mer�leges t�vols�g kisz�m�t�sa a pontt�l
                    float distanceToLine = distance(p, linePoint1, linePoint2);

                    // Ha ez a t�vols�g a jelenlegi legkisebb t�vols�gn�l kisebb, akkor friss�tj�k
    ...
    {
        vec3 dir = linePoint2 - linePoint1; // Az egyenes ir�nyvektora
        vec3 v = p - linePoint1; // A pont �s az egyenes kezd�pontja k�z�tti vektor
...
{// Convert to normalized device space
    ...
    switch (state) {
        case GLUT_DOWN: {
            buttonStat = "pressed";
            if (key_pressed[2]) {
                nearestLineIndex = lines->near_line(vec3(cX, cY, 1)); // legk�zelebbi egyenes index�nek lek�r�se
                if (nearestLineIndex != -1) { // ha van legk�zelebbi egyenes
                    lines->setClicked(p1, p2,
                                      nearestLineIndex); // kiv�lasztott egyenes kezd�- �s v�gpontjainak be�ll�t�sa
    ...
    switch (button) {
            case GLUT_LEFT_BUTTON: {
               // printf("Left button %s at (%3.2f, %3.2f)\n", buttonStat, cX, cY);
                ...
                {
                    if (p1.x != 0 && p1.y != 0) { // ellen�rizd, hogy m�r kiv�lasztott�l-e egy egyenest
                        nearestLineIndex = lines->near_line(vec3(cX, cY, 1)); // legk�zelebbi egyenes index�nek lek�r�se
                        if (nearestLineIndex != -1)
                        { // ha van legk�zelebbi egyenes
                    ...
                    } else { // ha m�g nem v�lasztott�l ki egy egyenest sem
                        nearestLineIndex = lines->near_line(vec3(cX, cY, 1)); // legk�zelebbi egyenes index�nek lek�r�se
                        if (nearestLineIndex != -1) { // ha van legk�zelebbi egyenes
                            lines->setClicked(p1, p2, nearestLineIndex); // kiv�lasztott egyenes kezd�- �s v�gpontjainak be�ll�t�sa
